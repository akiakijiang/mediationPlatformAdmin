...
class CommodityModel extends Model {
    ...
    public function get_tuijian(){
        ...
        shuffle($arr);
        for ($i = 0;$i<3;$i++){
            $data[] = $this->get_goods($temp[$arr[$i+1]]);
        }
     ...
     * @return array
     */
    public function get_goods($id,$array=array('commodityid','shopid','integral','title','thumbnail','original','current','carrousel','content','sales','carriageid','huohao'),$type=0){
        $key = C('REDIS_KEY').":goods:".$id;
        if(!$this->redis->exists($key)){
            $map['commodityid'] = $id;
     ...
     * @param $orderid
     */
    public function save_sku($orderid,$uid){
        ...
        foreach ($data as $k => $v){
            ...
            $M_sku->where($sku)->setDec('stock',$v['nums']);//库存-1
            $key = C('REDIS_KEY').":sku:".md5($sku['attr'].$sku['commodityid']);//加密下键
            $this->redis->del($key);//删除sku的redis缓存
            $this->where(array('commodityid'=>$sku['commodityid']))->setInc('sales',$v['nums']);//销量+1
            $key = C('REDIS_KEY').":goods:".$sku['commodityid'];
            $this->redis->del($key);//删除商品redis缓存
            $key = C('REDIS_KEY').":stock:".$sku['commodityid'];
...
//        $this->redis->del($key);
    ...
    public function get_default_carriage($id){
        $map['carriageid'] = $id;
        return M('carriage')->where($map)->cache(C('REDIS_KEY').":goodscarriage:".$id,400)->getField('price');
    }
